...
- [ ] Node.js, Expo Go, projeto Expo criado e rodando no celular
- [ ] Dependências instaladas (`expo-camera`, `expo-image-picker`, `expo-image-manipulator`, `expo-sqlite`, `expo-file-system`, `@react-native-community/netinfo`, `@supabase/supabase-js`, `@react-native-async-storage/async-storage`, `react-native-url-polyfill`)
- [ ] Projeto Supabase criado, tabelas `analises` e `itens_inventario` criadas com o SQL definido, bucket `fotos-inventario` criado
- [ ] Projeto Google Cloud criado, faturamento ativado (cartão, sem pré-pagamento), Cloud Vision API ativada, chave gerada e testada via curl
- [ ] `.env` configurado com `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, `EXPO_PUBLIC_GOOGLE_VISION_API_KEY`
